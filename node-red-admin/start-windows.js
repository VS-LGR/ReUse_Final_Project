...
// Verificar se o Node-RED está instalado
...
const startNodeRed = () => {
    ...
    // Usar npx para executar o Node-RED no Windows
    const nodeRed = spawn('npx', ['node-red', '--userDir', './data'], {
        stdio: 'inherit',
        cwd: __dirname,
        shell: true
    });
    ...
    nodeRed.on('error', (err) => {
        ...
        const altNodeRed = spawn('node', [path.join(__dirname, 'node_modules', 'node-red', 'red.js'), '--userDir', './data'], {
            stdio: 'inherit',
            cwd: __dirname,
            shell: true
        });
        
        altNodeRed.on('error', (altErr) => {
            console.error('❌ Erro no método alternativo:', altErr.message);
            console.log('\n🔧 Soluções possíveis:');
            console.log('1. Execute: npm install');
            console.log('2. Execute: npx node-red --userDir ./data');
            console.log('3. Verifique se o Node.js está instalado corretamente');
            process.exit(1);
        });
    });
